...
Language.snippets = [
	...
	{ input : 'q', output : '$sql = "SELECT * FROM $0";\n$result = mysql_query($sql) or die($sql."<br />".mysql_error());\n'},
	...
	{ input : 'sfcss', output : '<?php echo stylesheet_tag(\'$0\');?>' },
	{ input : 'sfjs', output : '<?php echo javascript_include_tag(\'$0\');?>' },
	{ input : 'sfq', output : '$sql = "SELECT * FROM $0";\n$stmt = $connection->prepareStatement($sql);\n$resultSet = $stmt->executeQuery();' },
	{ input : 'sffetch', output : 'while($resultSet->next()){\n $$=$resultSet->getString(\'$\');\n}' },
	{ input : 'sfimg', output : '<?php echo image_tag(\'$0\');?>' },
	{ input : 'sflink', output : '<?php echo link_to(\'$0\');?>' },
	...
	{ input : 'jexec', output : 'defined(\'_JEXEC\') or die(\'Restricted access\');' },
	{ input : 'jq', output : '$db = & JFactory::getDBO();\n$query = "SELECT * FROM #__$";\n$db->setQuery($query);' },
	{ input : 'jfetch', output : '$result = $db->loadObjectList();' },
	{ input : 'jimp', output : 'jimport(\'joomla.application.component.$0\');' },
	{ input : 'jass', output : '$this->assignRef(\'$0\',\$$);' },
	{ input : 'ado', output : '<audio src="$0"></audio>' },
	{ input : 'vdo', output : '<video src="$0"></video>' },
	{ input : 'sec', output : '<section>$0</section>' },
	{ input : 'nav', output : '<nav>$0</nav>' },
	{ input : 'footr', output : '<footer>$0</footer>' },
	{ input : 'headr', output : '<header>$0</header' },
	...
	{ input : 'fetch', output : 'while(\$row = @mysql_fetch_object(\$result)){\n\n}'},
